    ...
    static editById(req, res, next) {
        if(req.session.loggedToken) {
            return new Orm().getOrm().userModel // Getting logged User by Auth token
                ...
                .then((row, err) => {
                    let loggedUser = row.toJSON();
                    return new Orm().getOrm().faqsModel // Getting news which we will edit
                        .getById(req.params.id)
                        .then((row, err) => {
                            let faqs = row.toJSON();
                            req.body.id = faqs.id; // Add id to data

                            return new Orm().getOrm().faqsModel // Updating
                                .update(req.body)
                                .then((row) => { // Response with OK status
                                    let updFAQ = row.toJSON();
